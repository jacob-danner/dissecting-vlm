...
def validate_database(db_path: str = "vlm_analysis.db") -> bool:
    ...
    invalid = conn.execute("""
        ...
        HAVING prob_sum < 0.89 OR prob_sum > 1.0;
    """).fetchall()

    if invalid:
        print(f"   ✗ FAILED: {len(invalid)} positions have invalid probability sums")
    ...
    if embedding_count != expected_embeddings:
        print(f"   ✗ FAILED: Expected {expected_embeddings} embeddings, found {embedding_count}")
        all_passed = False
    else:
